...
class clusterSolution:
    def dfs(self, grid, i, j,prev):
        if i<0 or j<0 or i>=len(grid) or j>=len(grid[0]) or grid[i][j] != ('1' or '0') or prev == '0':
            return
        grid[i][j] = '#'
        self.dfs(grid, i, j+1,grid[i][j])
        self.dfs(grid, i+1, j+1,grid[i][j])
        self.dfs(grid, i+1, j,grid[i][j])
        self.dfs(grid, i+1, j-1,grid[i][j])
        self.dfs(grid, i, j-1,grid[i][j])
        self.dfs(grid, i-1, j-1,grid[i][j])
        self.dfs(grid, i-1, j,grid[i][j])
